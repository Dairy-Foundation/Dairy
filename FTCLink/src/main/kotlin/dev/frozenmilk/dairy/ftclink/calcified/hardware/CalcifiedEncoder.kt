...
abstract class CalcifiedEncoder<T> internal constructor(internal val module: CalcifiedModule, internal val port: Byte) {
	...
	abstract val positionSupplier: CompoundSupplier<T>
	abstract val velocitySupplier: CompoundSupplier<Double>
}

class TicksEncoder internal constructor(module: CalcifiedModule, port: Byte) : CalcifiedEncoder<Int>(module, port) {
	override val positionSupplier: CompoundSupplier<Int> = object : CompoundSupplier<Int> {
	...
	override val velocitySupplier: CompoundSupplier<Double> = object : CompoundSupplier<Double> {
...
class RadiansEncoder internal constructor(ticksEncoder: TicksEncoder, ticksPerRevolution: Double) : UnitEncoder<AngleRadians>(ticksEncoder, ticksPerRevolution) {
	override val positionSupplier: CompoundSupplier<AngleRadians> = object : CompoundSupplier<AngleRadians> {
		...
		override fun get(): AngleRadians {
			if (cachedAngle == null) cachedAngle = AngleRadians(Math.PI * 2 * (ticksEncoder.positionSupplier.get() / ticksPerRevolution))
			return cachedAngle!!
		}
	}
...
class DegreesEncoder internal constructor(ticksEncoder: TicksEncoder, ticksPerRevolution: Double) : UnitEncoder<AngleDegrees>(ticksEncoder, ticksPerRevolution) {
	override val positionSupplier: CompoundSupplier<AngleDegrees> = object : CompoundSupplier<AngleDegrees> {
		...
		override fun get(): AngleDegrees {
			if (cachedAngle == null) cachedAngle = AngleDegrees(360 * (ticksEncoder.positionSupplier.get() / ticksPerRevolution))
			return cachedAngle!!
		}
	}
